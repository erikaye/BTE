...
REAL*8, DIMENSION(:,:), ALLOCATABLE :: MatABCD_t1, MatEF_t1, MatGH_t1, VecJK_t1, MatABCD_t2, MatEF_t2, MatGH_t2, VecJK_t2, &
											tempMat, tempMatFull
REAL*8, DIMENSION(:,:,:), ALLOCATABLE :: AmatX, fX1Coeffs, fX2Coeffs, fX3Coeffs, fX1Coeffs_d, fX2Coeffs_d
										
REAL*8, DIMENSION(:,:,:), ALLOCATABLE :: MatABCD, MatEF, MatGH, VecJK
...
!!!!!!!!!! defining boundary conditions !!!!!!!!!!!!!!!!!!111


ALLOCATE(P1wvec(numW(1),1),BEndwvec(numW(nL),1))
P1wvec(:,1) = matlParams(matlsInds(1))%C(1:numW(1)) *dT1;
BEndwvec(:,1) = matlParams(matlsInds(2))%C(1:numW(nL)) *dT2;
	! last layer is of material 2
...
weights2(:,1) = matlParams(matlsInds(2))%weights(1:w2);
numEachFreq1 = matlParams(matlsInds(1))%numEachFreq(1:w1d);
numEachFreq2 = matlParams(matlsInds(2))%numEachFreq(1:w2d);
! material nL = material 2
...
ALLOCATE(MatABCD(maxNumRows,maxNumRows,nL-1), MatEF(maxNumRows,maxNumRows/2,nL-1), VecJK(maxNumRows,1,nL-1))
...
ALLOCATE(MatABCD_t1(numRows,numRows), MatEF_t1(numRows,numRows1), MatGH_t1(numRows,numRows0), VecJK_t1(numRows,1), &
		 MatABCD_t2(numRows,numRows), MatEF_t2(numRows,numRows0), MatGH_t2(numRows,numRows1), VecJK_t2(numRows,1), &
		 AmatX(N+1,N+1,2),fX1Coeffs(N+1,wMax,2),fX2Coeffs(N+1,wMax,2),fX3Coeffs(N+1,1,2))
		 
MatABCD_t1 = 0; MatEF_t1 = 0; MatGH_t1 = 0; VecJK_t1 = 0;
MatABCD_t2 = 0; MatEF_t2 = 0; MatGH_t2 = 0; VecJK_t2 = 0;
AmatX = 0; fX1Coeffs = 0; fX2Coeffs = 0; fx3Coeffs = 0;

! don't get a huge speed up... also there are race conditions that happen
! $OMP PARALLEL NUM_THREADS(2)
! $OMP SECTIONS
! $OMP SECTION		 
CALL GetInterfaceBM((/ matlParams(matlsInds(1)),matlParams(matlsInds(2)) /),Ls(1:2),Qs(1:2),skD, spec1, spec2, &
		G12_s, R12_s, T12_s, G21_s, R21_s, T21_s, G12_d, R12_d, T12_d, G21_d, R21_d, T21_d, &
		MatABCD_t1, MatEF_t1, MatGH_t1, VecJK_t1, &
...
! $OMP SECTION
CALL GetInterfaceBM((/ matlParams(matlsInds(2)),matlParams(matlsInds(1)) /),Ls(2:1:-1),Qs(2:1:-1),skD, spec1, spec2, &
		G21_s, R21_s, T21_s, G12_s, R12_s, T12_s, G21_d, R21_d, T21_d, G12_d, R12_d, T12_d, &
		MatABCD_t2, MatEF_t2, MatGH_t2, VecJK_t2, &
		AmatX(1:N+1,1:N+1,2), fX1Coeffs(1:N+1,1:w2,2), fX2Coeffs(1:N+1,1:w2,2), fX3Coeffs(1:N+1,:,2), &
		AmatX(1:N+1,1:N+1,1), fX1Coeffs(1:N+1,1:w1,1), fX2Coeffs(1:N+1,1:w1,1), fX3Coeffs(1:N+1,:,1))
! $OMP END SECTIONS
! $OMP END PARALLEL	
...
MatABCD(1:numRows,1:numRows,1) = MatABCD_t1;
MatEF(1:numRows,1:numRows1,1) = MatEF_t1;
VecJK(1:numRows,:,1) = VecJK_t1 + MATMUL(MatGH_t1,B1P2s(1:(w1+w1d),:,0))  ! w1 = numFreq first layer; w0 = 0
PRINT *, 'mats layer 1 done'

! can't parallelize bc recursive
DO m = 2,nL-1
	...
	IF (MODULO(m,2)==0) THEN  ! if even layer
		ALLOCATE(tempMat(numRows,w1+w1d),tempMatFull(numRows,numRows))
		tempMat = 0.0; tempMatFull = 0.0;
		tempMat = cBLOCK_MATMUL(MatGH_t1, & 
						cAXB(MatABCD(1:numRows0,1:numRows0,m-1),MatEF(1:numRows0,1:w1+w1d,m-1)),(/0,w1,0,w1d/),(/w0,w1,w0d,w1d/));
		...
		MatABCD(1:numRows,1:numRows,m) = MatABCD_t2 - tempMatFull;
		MatEF(1:numRows,1:w2+w2d,m) = MatEF_t2;
		VecJK(1:numRows,:,m) = VecJK_t2 + cBLOCK_MATMUL(MatGH_t2, &
							cAXB(MatABCD(1:numRows0,1:numRows0,m-1),VecJK(1:numRows0,:,m-1)),(/0,w1,0,w1d/),(/w0,w1,w0d,w1d/));
	...
	ELSE  ! if odd layer
		...
		tempMatFull(1:numRows,1:w1) = tempMat(1:numRows,1:w1)
		tempMatFull(1:numRows,w1+w2+1:w1+w2+w1d) = tempMat(1:numRows,w1+1:w1+w1d)
		MatABCD(1:numRows,1:numRows,m) = MatABCD_t1 - tempMatFull;
		MatEF(1:numRows,1:w2+w2d,m) = MatEF_t1;
		VecJK(1:numRows,:,m) = VecJK_t1 + cBLOCK_MATMUL(MatGH_t1, &
						cAXB(MatABCD(1:numRows0,1:numRows0,m-1),VecJK(1:numRows0,:,m-1)),(/0,w1,0,w1d/),(/w0,w1,w0d,w1d/));
		DEALLOCATE(tempMat, tempMatFull)
	END IF
...
DEALLOCATE(MatABCD_t1,MatEF_t1,MatGH_t1,VecJK_t1, MatABCD_t2,MatEF_t2,MatGH_t2,VecJK_t2)
...
! can't parallelize bc recursive (B1P2s)
DO m = nL-1,1,-1
	w0 = numW(m-1); w0d = numWd(m-1);
	w1 = numW(m); w1d = numWd(m); 
	w2 = numW(m+1); w2d = numWd(m+1);
	...
	B1P2s(1:numRows,:,m) = cAXB(MatABCD(1:numRows,1:numRows,m), &  ! square matrix
									cBLOCK_MATMUL(MatEF(1:numRows,1:w2+w2d,m),B1P2s(1:numRows1,:,m+1),(/0,w2,0,w2d/),(/0,w2,w3,w2d/)) & 
													+ VecJK(1:numRows,:,m))
	! B1P2s(1:numRows,:,1) = cAXB(MatABCD(1:numRows,1:numRows,1), &  ! square matrix
					! MATMUL(MatEF(1:numRows,1:w2+w2d,1),B1P2s(1:(w2+w2d),:,nL)) + VecJK(1:numRows))

	! ! specular part
	! B1P2s(1:(w1+w2),:,1) = cAXB(MatABCD1(1:(w1+w2),1:(w1+w2)), &  ! square matrix
					! MATMUL(MatEF1(1:(w1+w2),1:w2),B1P2s(1:w2,:,nL)) + VecJK1(1:(w1+w2),:))
					
	! ! diffuse part
	! B1P2s((w1+w2+1):(numRows),:,1) = cAXB(MatABCD1((w1+w2+1):(numRows),(w1+w2+1):(numRows)), &  ! square matrix
					! MATMUL(MatEF1((w1+w2+1):(numRows),(w2+1):(w2+w2d)),B1P2s((w2+1):(w2+w2d),:,nL)) + VecJK1((w1+w2+1):(numRows),:))
	...
	matN = 2-MODULO(m,2);		! 1 = mat1; 2 = mat2;
	B1wvecs_d(1:w1,1,m) = cDIFFVECtoFULL(tempB1_d(1:w1d,1,m), w1, matlParams(matlsInds(matN))%numEachFreq(1:w1d));
	matN = 2-MODULO(m+1,2);
	P2wvecs_d(1:w2,1,m) = cDIFFVECtoFULL(tempP2_d(1:w2d,1,m), w2, matlParams(matlsInds(matN))%numEachFreq(1:w2d));
...
DO m = 1,nL-1
	...
	matErr = MATMUL(MatABCD(1:numRows,1:numRows,m),B1P2s(1:numRows,:,m)) - &
				cBLOCK_MATMUL(MatEF(1:numRows,1:(w2+w2d),m),B1P2s(1:numRows,:,m+1),(/0,w2,0,w2d/),(/0, w2,w3,w2d/)) - VecJK(1:numRows,:,m)
...
DO m = 1,nL
	...
	w1 = numW(m)
	w1d = numWd(m)
	matN = 2-MODULO(m,2);  ! m odd = layer 1 of unit cell, m even = layer 2 of unit cell
	dTCoeffs(:,m) = RESHAPE(cAXB(AmatX(:,:,matN), &
						(MATMUL(fX1Coeffs(:,1:w1,matN), P2wvecs(1:w1,:,m-1)) + &
						 MATMUL(fX2Coeffs(:,1:w1,matN), B1wvecs(1:w1,:,m)) + &
						fX3Coeffs(:,:,matN))),(/w1/))
	dTCoeffs_s(:,m) = RESHAPE(cAXB(AmatX(:,:,matN), &
						(MATMUL(fX1Coeffs(:,1:w1,matN), P2wvecs_s(1:w1,:,m-1)) + &
						 MATMUL(fX2Coeffs(:,1:w1,matN), B1wvecs_s(1:w1,:,m)) + &
						fX3Coeffs(:,:,matN))),(/w1/))
	dTCoeffs_d(:,m) = RESHAPE(cAXB(AmatX(:,:,matN), &
						(MATMUL(fX1Coeffs(:,1:w1,matN), P2wvecs_d(1:w1,:,m-1)) + &
						 MATMUL(fX2Coeffs(:,1:w1,matN), B1wvecs_d(1:w1,:,m)) + &
						fX3Coeffs(:,:,matN))),(/w1/))
END DO 
...
DO m = 1,nL-1
	...
	IF (m1==1) THEN	! mat 1 -> mat 2
		...
		CALL CheckBCs_BBbc( fileNum, write_path, (/ matlParams(matlsInds(m1)),matlParams(matlsInds(m2)) /), &
							 Qs(m:m+1), skD, Ls((/m1,m2/)), SUM(Ls)*numPer, numPer, &
							 fX1Coeffs(:,:,(/m1,m2/)), fX2Coeffs(:,:,(/m1,m2/)), &
							 B1wvecs_s(:,:,m-1:m+1), B1wvecs_d(:,:,m-1:m+1), P2wvecs_s(:,:,m-1:m+1), P2wvecs_d(:,:,m-1:m+1), &
							 G12_s, R12_s, T12_s, G21_s, R21_s, T21_s, G12_d, R12_d, T12_d, G21_d, R21_d, T21_d, &
							 fBoth(:,(/m1,m2/)), numFBoth, weightsMatch, &
...
DEALLOCATE(P1wvec, BEndwvec, B1P2s, B1wvecs, P2wvecs, tempB1_d, tempP2_d, &
		   B1wvecs_s, B1wvecs_d, P2wvecs_s, P2wvecs_d)
DEALLOCATE(AmatX, fX1Coeffs, fX2Coeffs, fX3Coeffs, MatABCD, MatEF, VecJK, dTCoeffs, dTCoeffs_s, dTCoeffs_d, cosMat, xs)

dT = dTtemp

DEALLOCATE(dTtemp)

END SUBROUTINE MultilayerT_BBC_fct_noQ