...
REAL*8, DIMENSION(:,:), ALLOCATABLE :: temp1_s, temp2_s,temp1_d, temp2_d, qInttemp1, qInttemp2
REAL*8, DIMENSION(:,:,:), ALLOCATABLE :: bcErr_s, bcErr_d, bcErr
...
ALLOCATE(bcErr_s(w2+w1,1,nL-1), bcErr_d(w2+w1,1,nL-1), bcErr(w2+w1,1,nL-1), intGs(MAX(w1,w2),1,nL-1,8), intQs(MAX(w1d,w2d),1,nL-1,4))
...
DO m = 1,nL-1
	...
	! matlParams inputs are already ordered (eg. Si/MgO or MgO/Si)
	mat1 = m;
	mat2 = m + 1;
	...
	ALLOCATE(kx1(w1,1),kx2(w2,1),ky1(w1,1),ky2(w2,1),kz1(w1,1),kz2(w2,1), freq1(w1,1), freq2(w2,1), vgx1(w1,1),vgx2(w2,1), tau1(w1,1), tau2(w2,1), &
				...
				term3_1f(w1),term3_2b(w2),phi2b(N+1,w2),phi1f(N+1,w1),phi2b_dT(w2,N+1),phi1f_dT(w1,N+1))
	...
	g2m0wTerm_d(1:w2,:,m) = scSPARSE_MATMUL_L(G21_d,(B1wvecs_d(1:w2,:,m+2)*vgx2*EXP(-gamma_mu2))*weights2,w2,1) + temp2_d*weights2;
	g2m0wTerm_s(1:w2,:,m) = (B1wvecs_s(1:w2,:,m+2)*vgx2*EXP(-gamma_mu2) + temp2_s)*weights2;
	...
	g2p0wTerm_d(1:w2,:,m) = scSPARSE_MATMUL_L(G21_d,P2wvecs_d(1:w2,:,m+1)*vgx2*weights2,w2,1);
	g2p0wTerm_s(1:w2,:,m) = P2wvecs_s(1:w2,:,m+1)*vgx2*weights2;
	...
	DEALLOCATE(kx1,kx2,ky1,ky2,kz1,kz2, freq1,freq2, vgx1,vgx2, tau1,tau2, &
			...
			term3_1f, term3_2b, phi2b, phi1f, phi2b_dT, phi1f_dT)
...
DEALLOCATE(bcErr,bcErr_d,bcErr_s,intGs,intQs)
