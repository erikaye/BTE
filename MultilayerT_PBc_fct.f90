...
IF (filesExist) THEN		! read the files
	...
	! ! !$OMP SECTION
		...
		spec1(:,1) = matlParams(matlsInds(1))%spec(1:w1);
		spec2(:,1) = matlParams(matlsInds(2))%spec(1:w2);
...
! layer 1/2
CALL GetInterfaceBM_PBC((/ matlParams(matlsInds(1)),matlParams(matlsInds(2)) /),Ls(1:2),Qs(1:2),skD, spec1, spec2, 0.0, &
		G12_s, R12_s, T12_s, G21_s, R21_s, T21_s, G12_d, R12_d, T12_d, G21_d, R21_d, T21_d, &
...
! layer 0/1
CALL GetInterfaceBM_PBC((/ matlParams(matlsInds(2)),matlParams(matlsInds(1)) /),Ls(2:1:-1),Qs(2:1:-1),skD, spec2, spec1, dT2-dT1, &
...
! write spec1 to a txt file
OPEN(5,FILE = './output_spec1_' // fileNum // '.dat')
WRITE(5,"(40000(ES15.7,','))") spec1
CLOSE(5)

! ! write actual spec to a txt file
! OPEN(1,FILE = TRIM(write_path) // 'output_specRealB_' // fileNum // '.dat')
! OPEN(2,FILE = TRIM(write_path) // 'output_specRealF_' // fileNum // '.dat')
! DO m = 1, nL
	! WRITE(1,"(40000(ES16.7E3,','))") specRealB(:,1,m)
	! WRITE(2,"(40000(ES16.7E3,','))") specRealF(:,1,m)
! END DO
! CLOSE(1)
! CLOSE(2)
...
DO m = 1,nL
	...
	IF (m1==1) THEN	! mat 1 -> mat 2
		...
		CALL CheckBCs_Pbc( fileNum, write_path, (/ matlParams(matlsInds(1)),matlParams(matlsInds(2)) /), &
							 0.0, Qs(1:2), skD, Ls((/1,2/)), SUM(Ls)*numPer, numPer, &
	...
	ELSE
		...
		CALL CheckBCs_Pbc( fileNum, write_path, (/ matlParams(matlsInds(2)),matlParams(matlsInds(1)) /), &
							 dT2-dT1, Qs((/2,1/)), skD, Ls((/2,1/)), SUM(Ls)*numPer, numPer, &
...
END SUBROUTINE MultilayerT_PBC_fct