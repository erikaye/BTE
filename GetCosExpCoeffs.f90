...
DO m = 0,N    ! inhomogenous kernel f coefficients 
	...
	fx1Coeffs(m+1,:) = 2.0/norm/tau/denom_m*(1.0-((-1.0)**m)*EXP(-gamma_mu))*weights
	fx2Coeffs(m+1,:) = 2.0/norm/tau/denom_m*(((-1.0)**m)-EXP(-gamma_mu))*weights
...
DO p = 0,N
	...
	DO m = 0,N
		...
		kxCoeffs(m+1,p+1) = 4.0/norm*term1;
